...
impl Default for DateBasedQuery {
    fn default() -> Self {
        ...
        DateBasedQuery {
            ...
            to_date: now.with_year(now.year() + 1).unwrap()
...
fn handle_schedule_request_v2(query: web::Query<DateBasedQuery>) -> Result<HttpResponse, HazizzError> {
    let request_started = Instant::now();

    let lessons_sorted = get_schedule_v2(&query.token, &query.url, &query.from_date, &query.to_date)?;
...
fn handle_schedule_request(query: web::Query<DateBasedQuery>) -> Result<HttpResponse, HazizzError> {
    ...
    let lessons: Vec<Lesson> = get_schedule(&query.token, &query.url, &query.from_date, &query.to_date)?;
...
fn main() {
    ...
    HttpServer::new(|| App::new()
        ...
        .service(web::resource("/v2/schedules").route(web::get().to(handle_schedule_request_v2)))
